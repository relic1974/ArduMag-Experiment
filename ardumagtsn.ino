...
void loop()
{
  if(offsetsFinished == true)
  {
    offsetsFinished = false;
    startMission();
  }
}
...
void startMission()
{
    ...
    while(charCount <= 9999)                  // 18.5 orbits
    {
      x = mag.readx();
      y = mag.ready();
      z = mag.readz();
      ...
      lineBuffer[written] = '\0';            // Make sure the buffer is null terminated
      charCount += written;
      
      storage.send(lineBuffer);              // Append the data to SD card
      
      delay(180000);                            // change to 3 minutes delay 180000
